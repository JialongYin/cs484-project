...
#define MPI_PRINT(content) if (0 == rank) std::cout << content
#define MPI_ERROR(content) if (0 == rank) std::cerr << content
...
int main(int argc, char **argv) {
	...
	// local_N = chooseArraySize(cmdln_size_dist_choice,size_dist_a,size_dist_b) % 500 + 10;
	...
	MPI_Allreduce(&local_N, &global_N, 1, MPI_TYPE_DIST_SORT_SIZE_T, MPI_SUM, MPI_COMM_WORLD);
	...
	if (0 == rank) {
		...
		std::cout << std::endl;
		std::cout << "RESULTS: Total_Data : " << global_N << std::endl << std::flush;
		std::cout << "RESULTS: Duration_Rebalance " << (rebalance_time).count() << " s" << std::endl << std::flush; total_time+=rebalance_time;
		std::cout << "RESULTS: Duration_Sort_1 " << (sort_time_1).count() << " s" << std::endl << std::flush; total_time+=sort_time_1;
		std::cout << "RESULTS: Duration_Splitters " << (splitters_time).count() << " s" << std::endl << std::flush; total_time+=splitters_time;
		std::cout << "RESULTS: Duration_Move " << ( movedata_time).count() << " s" << std::endl << std::flush; total_time+=movedata_time;
		std::cout << "RESULTS: Duration_Sort_2 " << (sort_time_2).count() << " s" << std::endl << std::flush; total_time+=sort_time_2;
		std::cout << "RESULTS: Duration_Total " << (total_time).count() << " s" << std::endl << std::flush;
	}
